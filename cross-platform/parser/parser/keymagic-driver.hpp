...
public:
	...
	typedef std::map<std::wstring, int > VARINDEXS;
	VARINDEXS varindexs;
	...
	bool trace_scanning;
	//
	bool StoreSwitch(const std::wstring& name);
	int IndexOfSwitch(const std::wstring& name, yy::location l);
	bool StoreVariable(const std::wstring& name, const std::list<int>& value, yy::location l);
	std::list<int>& LoadVariable(const std::wstring& name, yy::location l);
	int IndexOfVariable(const std::wstring& name, yy::location l);
	...
	std::string * U16toU8(const std::wstring& u16);
	std::string * U16toU8(int u16);
	//
	bool AddInfo(int id, const char * data, short size);
	bool SetIcon(const std::string& path);
	//
	bool StringToBool(std::wstring& booleanStr) ;
	bool StringToBool(std::string& booleanStr) ;
	// Run the parser.  Return 0 on success.
	int parse (const std::string& f);
	bool openFile (const std::string& f);
	bool openIncludeFile(const std::string& from, const std::string& incFile, std::string * realPath);
	std::string file;
	bool trace_parsing;
	// Error handling.

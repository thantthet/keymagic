...
typedef std::vector<structRule> RULELIST;
...
public:
	...
	bool fromFile(const wchar_t * szPath)
	{
		...
		if (fh.bMajorVersion > 1) {
			return false;
		} else if (fh.bMajorVersion == 1 && fh.bMinorVersion > 4) {
			return false;
		}

		if (fh.bMajorVersion == 1 && fh.bMinorVersion == 3) {
			FileHeader13 fh13;
			fseek(hFile, 0, SEEK_SET);
			fread(&fh13, sizeof(fh13), 1, hFile);
			fh.layout = fh13.layout;
			fh.shNumOfRules = fh13.shNumOfRules;
			fh.shNumOfString = fh13.shNumOfString;
			fh.shNumOfInfos = 0;
		}
		...
		for (int i = 0; i < fh.shNumOfInfos; i++)
		{
			int id;
			fread(&id, sizeof(id), 1, hFile);
			short sLength;
			fread(&sLength, sizeof(sLength), 1, hFile);
	...
	bool toFile(const wchar_t * szPath)
	{
		...
		for (RULELIST::iterator it = rxRules.begin();
			it != rxRules.end();
			it++)
		{

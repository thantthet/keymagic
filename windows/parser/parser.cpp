...
wchar_t * parser::identifier(int * objIndex)
{
	...
	if (!varvalue)
	{
		...
		Exit(1, L"ERROR: Unknown identifier '%s' at Line=%d Pos=%d\n", varName, Script.getLineNum(pos), Script.getPosLine(pos));
		return false;
	}
...
typedef std::map<boost::wregex, void*> MapOptions;
...
bool parser::begin_parse()
{
	...
	MapOptions options;
	options[reCaps] = &kmklf.layout.trackCaps;
	options[reUnused] = &kmklf.layout.eat;
	options[reUSLayout] = &kmklf.layout.posBased;
	
	for (MapOptions::iterator iter = options.begin(); iter != options.end(); iter++) {
		boost::wcmatch matches;
		if (boost::regex_search(Script.lpwStrAt(0), matches, (*iter).first)) {
			Debug(L"%s", (*iter).first.str());
			if (wcsncmp((matches[1].first),L"false",5)==0) {
				((bool*)((*iter).second))[0] = false;
			}
		}
	}
